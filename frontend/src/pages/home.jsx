...
import { Briefcase, Inbox, Plus, Search, Send, SlidersHorizontal, TrendingUp, X, Star } from "lucide-react";
...
export default function Home() {
  ...
  const detailWork = detailWorkId
    ? workPosts.find((w) => (w.id || w._id) === detailWorkId)
    : null;
  ...
  return (
    <div className="flex-1 overflow-auto pb-24">
      <div className="w-full px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="mb-5 overflow-hidden rounded-2xl border bg-card shadow-sm">
          <div className="grid gap-5 p-5 lg:grid-cols-[1fr_420px] lg:p-6">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                <TrendingUp className="h-3.5 w-3.5" />
                Live marketplace
              </div>
              <h1 className="max-w-3xl text-2xl font-semibold tracking-tight sm:text-3xl">
                Find skilled professionals and high-quality project work.
              </h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                Search by skill, compare budgets, inspect public profiles, and apply with a focused proposal.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-xl border bg-background p-3">
                ...
                <p className="text-xl font-semibold">{dashboard?.projectsPosted ?? 0}</p>
              </div>
              <div className="rounded-xl border bg-background p-3">
                <Send className="mb-2 h-4 w-4 text-emerald-600" />
                <p className="text-xs text-muted-foreground">Sent</p>
                <p className="text-xl font-semibold">{dashboard?.applicationsSent ?? appliedWorkIds.size}</p>
              </div>
              <div className="rounded-xl border bg-background p-3">
                <Inbox className="mb-2 h-4 w-4 text-amber-600" />
                <p className="text-xs text-muted-foreground">Received</p>
                <p className="text-xl font-semibold">{dashboard?.applicationsReceived ?? 0}</p>
              </div>
            </div>
          </div>
        </section>
              ...
              filtered.map((w) => {
                ...
                return (
                  <WorkCard
                    ...
                    work={w}
                    onApply={handleApply}
                    onViewDetails={(id) => setDetailWorkId(id)}
                    isApplied={isApplied}
                  />
                );
              })
      ...
      <Dialog open={!!detailWorkId} onOpenChange={(open) => !open && setDetailWorkId(null)}>
        <DialogContent className="max-h-[85vh] max-w-2xl bg-card border rounded-2xl">
          ...
          <ScrollArea className="max-h-[60vh] pr-4 my-2">
            {detailWork && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-lg font-semibold text-foreground">{detailWork.title}</h2>
                  <div className="mt-2 flex flex-wrap gap-2">
                    <Badge variant="secondary">{detailWork.category || "General"}</Badge>
                    {detailWork.projectLevel && (
                      <Badge variant="outline" className="capitalize">
                        {detailWork.projectLevel} level
                      </Badge>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 border-y py-4 sm:grid-cols-4">
                  <div>
                    ...
                    <span className="text-sm font-semibold text-foreground">₹{Number(detailWork.budget || 0).toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-xs text-muted-foreground block">Deadline</span>
                    <span className="text-sm font-semibold text-foreground">{detailWork.deadline ? new Date(detailWork.deadline).toLocaleDateString() : "No deadline"}</span>
                  </div>
                  <div>
                    <span className="text-xs text-muted-foreground block">Work Type</span>
                    <span className="text-sm font-semibold text-foreground capitalize">{detailWork.workType || "Remote"}</span>
                  </div>
                  <div>
                    <span className="text-xs text-muted-foreground block">Posted On</span>
                    <span className="text-sm font-semibold text-foreground">{detailWork.postedDate || detailWork.createdAt ? new Date(detailWork.postedDate || detailWork.createdAt).toLocaleDateString() : "Recently"}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed break-words">
                    {detailWork.fullDescription || detailWork.shortDescription || "No description provided."}
                  </p>
                </div>

                {detailWork.skills && detailWork.skills.length > 0 && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-semibold">Skills Required</h3>
                    <div className="flex flex-wrap gap-1.5">
                      {detailWork.skills.map((s) => (
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="flex items-center gap-3">
                    <Avatar className="h-12 w-12">
                      <AvatarImage src={detailWork.poster?.profileImage} />
                      <AvatarFallback className="text-sm">
                        {detailWork.poster?.name ? detailWork.poster.name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0,2) : "?"}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      <p className="text-sm font-semibold">{detailWork.poster?.name || "Client"}</p>
                      <p className="text-xs text-muted-foreground">{detailWork.poster?.email}</p>
                      <div className="flex items-center gap-1 mt-1">
                        <Star className="h-3.5 w-3.5 fill-yellow-500 text-yellow-500" />
                        <span className="text-xs font-medium">{detailWork.poster?.rating || 0} rating</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <DialogFooter className="border-t pt-4 gap-2 flex-row justify-end">
            <Button variant="outline" onClick={() => setDetailWorkId(null)}>
              Close
            </Button>
            {detailWork && detailWork.postedBy !== currentUserId && !appliedWorkIds.has(detailWork.id || detailWork._id) && (
              <Button onClick={() => {
                setDetailWorkId(null);
                handleApply(detailWork.id || detailWork._id);
              }}>
                Apply to Project
              </Button>
            )}
            {detailWork && appliedWorkIds.has(detailWork.id || detailWork._id) && (
              <Button disabled className="bg-green-600 hover:bg-green-600 text-white cursor-not-allowed">
                Applied
              </Button>
            )}
          </DialogFooter>
