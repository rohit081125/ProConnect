...
export default function Messages() {
  ...
  useEffect(() => {
    const checkSupportOrInitial = async () => {
      if (initialUserId) {
        setSelectedUserId(initialUserId);
      } else if (params.get("support") === "true") {
        try {
          const adminResponse = await getAdminUser();
          const admin = adminResponse?.data || adminResponse;
          const adminId = getSafeId(admin);
          if (adminId) {
            setSelectedUserId(adminId);
          }
        } catch (error) {
          console.error("Failed to preselect admin support user:", error);
        }
      }
    };
    checkSupportOrInitial();
  }, [initialUserId, queryString]);
  ...
  // Polling for sidebar contacts' last messages periodically (even when no chat is open)
  useEffect(() => {
  ...
  }, [contacts, currentUserId, selectedUserId]);

  // Polling for the active chat messages
  useEffect(() => {
  ...
  const loadContacts = async () => {
    try {
      ...
      if (isAdmin) {
        try {
          const adminThreadsResponse = await getAdminChatThreads();
          const adminThreads = Array.isArray(adminThreadsResponse)
            ? adminThreadsResponse
            : adminThreadsResponse?.data || [];

          adminThreads.forEach((user) => {
          ...
          console.error("Failed to load admin support threads:", error);
        }
      } else {
        try {
          const adminResponse = await getAdminUser();
          ...
          if (adminId && adminId !== currentUserId) {
            contactMap.set(adminId, {
              id: adminId,
              name: admin?.name || "Rohit Donga",
              profileImage: admin?.profileImage || "",
              isSupportAdmin: true,
            });
          }
        } catch (error) {
          console.error("Failed to load admin user for support contact:", error);
        }
        ...
        // Fetch all general chat conversation partners
        try {
          const userThreadsResponse = await getUserThreads(currentUserId);
          const userThreads = Array.isArray(userThreadsResponse)
            ? userThreadsResponse
            : userThreadsResponse?.data || [];

          userThreads.forEach((partner) => {
            const partnerId = getSafeId(partner);
          ...
          console.error("Failed to load general chat threads:", error);
        }
      }
  ...
  const loadMessages = async (chatUserId = selectedUserId, showLoader = false) => {
    ...
    try {
      ...
      const hasUnread = data.some(msg => msg.senderId === chatUserId && msg.isRead !== true && msg.read !== true);
      if (hasUnread) {
        await handleMarkRead(chatUserId);
      }
    } catch (error) {
        ...
        contacts.map(async (contact) => {
          try {
            ...
            const unreadCount = data.filter(
              (msg) => msg.senderId === contact.id && msg.isRead !== true && msg.read !== true
            ).length;
            ...
            temp[contact.id] = {
              ...
              isNewFromOther:
                ...
                last.read !== true &&
                !isCurrentlyOpened,
              unreadCount: isCurrentlyOpened ? 0 : unreadCount,
            };
          } catch (error) {
            temp[contact.id] = {
              message: "",
  ...
  const sortedContacts = useMemo(() => {
    return [...contacts].sort((a, b) => {
      if (a.isSupportAdmin && !b.isSupportAdmin) return -1;
      if (!a.isSupportAdmin && b.isSupportAdmin) return 1;
          ...
          <ScrollArea className="flex-1">
            {loadingContacts ? (
              ...
              sortedContacts.map((contact) => {
                ...
                const hasUnread = preview.unreadCount > 0 || preview.isNewFromOther;
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          >
                            {contact.name}
                            {contact.isSupportAdmin && (
                              <span className="text-xs text-muted-foreground font-normal ml-0.5">(Admin)</span>
                            )}
                            {contact.isSupportAdmin && (
                              <Shield className="h-3.5 w-3.5 text-accent fill-accent/10 shrink-0" />
                            )}
                          </p>

                          <span className={`text-[10px] ${hasUnread ? "font-black text-foreground" : "text-muted-foreground"}`}>
                        ...
                        <div className="flex items-center justify-between gap-2 mt-0.5">
                          ...
                          {preview.unreadCount > 0 && (
                            <span className="h-2.5 w-2.5 rounded-full bg-red-500 shrink-0 border border-background" />
                          )}
                        </div>
                      </div>
                    </div>
